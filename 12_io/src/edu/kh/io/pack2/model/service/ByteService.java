...
public class ByteService {
	...
	// -> IO관련 코드는 거의 모든 메서드가 throw IOException
	...
	public void fileByteOutput() {
		...
		try {
			...
			// 출력방법 1 : 한 글자(2byte char형식)씩 파일로 출력
			...
			*/
			...
			try {
				...
				// clos() 메서드도 IOException을 발생시키기 때문에
	...
	public void bufferedFileByteOutput() {
		 ...
		 * 
		 * - FileOutputStream은 1바이트씩 데이트를 입출력
		...
		try {
			...
			bos.write(content.getBytes()); // 버퍼에 저장(파일에 즉시 기록하지 않음)
			
			long endTime = System.nanoTime();
			
			System.out.println("[수행 시간] : " + (endTime - startTime) + "ns"); // 38600ns
			
			bos.flush(); // 스트림 안에 남아있는 모든 데이터를 밀어냄
			// 버퍼 내용을 강제로 파일에 기록
			// BufferedOutputStream 사용 시 flush() 사용 필수
	...
	public void fileByteInput() {
		...
		try {
			...
			while(true) {
				...
				sb.append( (char)value ); // char로 강제 형변환해서 글자 형태로 만든 뒤 sb 추가
	...
	public void readProperties() {
		...
		// Key와 Value 모두 String 타입으로 이뤄져있다.
	 ...
	 * -> BufferedReader 생성 시 보조스트림인 IntputStreamReader -> InputStream가 기반
	 * 
	 */
	public void fileCopy() {
		...
		try {
			...
			//     입력 성능향상  <- 문자 스트링으로 변환 <- 1byte 씩 입력할 수 있게 해줌
			...
			StringBuilder sb = new StringBuilder();
			sb.append(target); // "io_test/20250305/노래가사_copy.txt"
			...
			// 출력용 스트림 생성 / /io_test/20250305/노래가사_copy.txt 전달
			fos = new FileOutputStream(copy);
			bos = new BufferedOutputStream(fos);
			
			// 원본에서 읽어온 내용 bytes 를 bos 를 이용해서 쓰기(출력)
			bos.write(byres);
