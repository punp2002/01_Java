...
public class StudentService {
	...
	public void ex() { // List 테스트용 예제
		...
		// Object List.get(index) : 리시트에서 index번째 인덱스에 있는 객체를 반환
	...
	public String addStudent() {
		// Checked Exception : 반드시 throws 구문 이나 try-catch 처리 강제
	 ...
	 * 3. 학생 정보 수정 메서드
	 * 
	 * - 학생정보가 studentList에 하나라도 있는지 검사,
	...
	public String removeStudent() {
		...
		// remve() 동작 시 뒤쪽 요소를 한칸씩 자동으로 당겨온다.
		System.out.print("인덱스 번호 : ");
		int index = sc.nextInt();
	...
	public void searchName2() {
		...
		for(Student std : studentList) {
			...
			// boolean String.cotains(문자열) : String에 문자열 포함되어 있으면 true/false
			if( std.getName().contains(input) ) { // std.getName() 에 input이 포함되어있는지 !!! 순서 유의
				System.out.println(std);
	 ...
	 * List를 정렬하는 방법
	 * 
	 * 방법 1: Comparable 인터페이스 상속 받아 compareTo() 메서드 재정의
	 * Student에 Comparable 인터페이스 를 상속 받아 오버라이딩 한 compareTo()에
	 * 정의한 대로 정렬됨 (나이 오름차순, 내림차순)
	 * 
	 * 방법 2 : Comparator 클래스에 의한 정렬 compare() 사용 (익명 내부 클래스 이용)
	 * 익명 내부 클래스 : 이름이 없는 클래스를 즉석에서 선언해서 한번만 사용할 목적으로 작성
	...
	public void sortByName() {
		...
		Collections.sort(studentList, new Comparator<Student>(){
			...
			public int compare(Student o1, Student o2) {
				...
				// (String 이 Comarable을 상속받아 재정의해둔 compareTo() 메서드를 이용하는 것
